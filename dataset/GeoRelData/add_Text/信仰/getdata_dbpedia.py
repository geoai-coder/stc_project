...
def getkeypos():
    ...
    ns_words_count_ = defaultdict(int)
    with open('filtered_output_select.txt', 'r', encoding='utf-8') as file1 ,\
            open('filtered_output_select_pos_nh.txt', 'w', encoding='utf-8') as file2,\
            open('filtered_output_select_pos_nz.txt', 'w', encoding='utf-8') as file3,\
        open('ns_words_count_nh.txt', 'w', encoding='utf-8') as dict_file_nh,\
        ...
        for line in file1:
            ...
            if any(keyword in line for keyword in keywords):
                ...
                if textpos != False and 'nz' in textpos and textpos.count('nz') > 1:
                    file3.write(line)
                    # 提取 ns 词性对应的词并统计出现次数
                    for word, pos in zip(textword, textpos):
                        if pos == 'nz':
                            ns_words_count[word] += 1
                if textpos != False and 'nh' in textpos and 'nz' in textpos:
                    file2.write(line)
                    # 提取 ns 词性对应的词并统计出现次数
                    for word, pos in zip(textword, textpos):
                        if pos == 'nh':
                            ns_words_count_[word] += 1
                print(f'第：{readnum}-{rate:.3%}')
        # 将 ns 词及其出现次数写入字典文件
        for word, count in ns_words_count.items():
            dict_file_nz.write(f'{word}: {count}\n')
        for word, count in ns_words_count_.items():
            dict_file_nh.write(f'{word}: {count}\n')
