import jieba
from Ctgu.Tools.LTP_ import *
from collections import defaultdict

keywords = ['GDP', 'GDP总计','国内生产总值']

def filter_and_split_file(input_file, output_prefix, keywords):
    try:
        readnum, wikinum = 1, 15360810
        output_file = f"{output_prefix}.txt"
        with open(input_file, 'r', encoding='utf-8') as infile,open(output_file, 'a', encoding='utf-8') as outfile:
            for line in infile:
                readnum += 1
                rate = readnum / wikinum
                if any(keyword in line for keyword in keywords):
                    outfile.write(line)
                print(f'第：{rate:.3%}')
    except Exception as e:
        print(f"发生错误: {e}")

def getfile():
    # 定义参数
    # input_file = '../../../../EncyclopediaData/wiki_split_cws/wikiSplit.txt'
    input_file = '../../../../EncyclopediaData/baidu and st _v2/baidu_split_v2.txt'
    output_prefix = 'filtered_output'
    # 运行函数
    filter_and_split_file(input_file, output_prefix, keywords)

def getkeywords():
    import jieba.analyse

    jieba.add_word("GDP")
    jieba.add_word("GDP总计")
    jieba.add_word("国内生产总值")
    keywords = ['GDP', 'GDP总计','国内生产总值']

    # 示例中文文档
    readnum,filelen = 0,27469
    with open('filtered_output.txt', 'r', encoding='utf-8') as file1,open('filtered_output_select.txt', 'w', encoding='utf-8') as file2:
        for line in file1:
            readnum += 1
            rate = readnum / filelen
            # 使用 TF-IDF 提取关键词
            keywords_tfidf = jieba.analyse.extract_tags(line, topK=10, withWeight=False)
            if len(line) >20 and any(element in keywords for element in keywords_tfidf):
                file2.write(line)
                # print("TF-IDF Keywords:", keywords_tfidf)
            print(f'第：{rate:.3%}')

def getkeypos():
    ns_words_count = defaultdict(int)
    with open('filtered_output_select.txt', 'r', encoding='utf-8') as file1 ,\
            open('filtered_output_select_pos.txt', 'w', encoding='utf-8') as file2,\
        open('ns_words_count.txt', 'w', encoding='utf-8') as dict_file:
        readnum, filelen = 0, 27469
        for line in file1:
            readnum += 1
            rate = readnum / filelen
            if any(keyword in line for keyword in keywords):
                # 使用 TF-IDF 提取关键词
                ltptext = LTP_(line.strip())
                textword,textpos = ltptext.ltpTowords()
                if textpos != False and 'ns' in textpos and 'm' in textpos:
                    file2.write(line)
                    # 提取 ns 词性对应的词并统计出现次数
                    for word, pos in zip(textword, textpos):
                        if pos == 'ns':
                            ns_words_count[word] += 1

                print(f'第：{readnum}-{rate:.3%}')
        # 将 ns 词及其出现次数写入字典文件
        for word, count in ns_words_count.items():
            dict_file.write(f'{word}: {count}\n')
def rewritetext():
    with open('filtered_output_select_pos.txt', 'r', encoding='utf-8') as file1,\
    open('filtered_output_select_pos_.txt', 'a', encoding='utf-8') as file2:
        fileline = 0
        for line1 in file1:
            mlist,inmlist = [],[]
            fileline+=1
            print(f"8462-{fileline}")
            if fileline < 90:
                if "目录" in line1:
                    print(fileline, line1)
                    continue

                else:
                    ltptext = LTP_(line1.strip())
                    textword, textpos = ltptext.ltpTowords()
                    for i,pos in enumerate(textpos):
                        if pos == 'm':
                            mlist.append(textword[i])
                    print(mlist)
                    for word in mlist:
                        if '%' not in word and '％' not in word:
                            inmlist.append(word)
                    print(inmlist)
                    if inmlist != []:
                        # file2.write(line1)
                        user_input = input(f"{line1} (y/n): ").strip().lower()
                        if user_input == 'n':
                            continue
                        else:
                            file2.write(line1)
                    else:
                        continue
                    # user_input = input(f"{line1} (y/n): ").strip().lower()
                    # if user_input == 'n':
                    #     continue
                    # else:
                    #     file2.write(line1)
                # if len(line1) > 800:
                #     user_input = input(f"{line1} (y/n): ").strip().lower()
                #     if user_input == 'n':
                #         continue
                #     else:
                #         file2.write(line1)
                # elif len(line1) < 30:
                #     user_input = input(f"{line1} (y/n): ").strip().lower()
                #     if user_input == 'n':
                #         continue
                #     else:
                #         file2.write(line1)
                # else:
                #     file2.write(line1)
def deletetext():
    with open('filtered_output_select_pos___.txt', 'r', encoding='utf-8') as file1,\
    open('filtered_output_select_pos____.txt', 'a', encoding='utf-8') as file2:
        fileline = 0
        for line1 in file1:
            fileline+=1
            print(f"7811-{fileline}")
            if  ('第' in line1 and '章' in line1):
                print('错误',line1)
                continue
            else:
                if len(line1) > 1000:
                    user_input = input(f"{len(line1)}-{line1} (y/n): ").strip().lower()
                    if user_input == 'n':
                        print('错误', line1)
                        continue
                    else:
                        file2.write(line1)
                else:
                    file2.write(line1)

def deletechinse():
    def contains_digit(s):
        # Check if any character in the string is a digit
        return any(char.isdigit() for char in s)
    with open('filtered_output_select_pos___.txt', 'r', encoding='utf-8') as file1,\
    open('filtered_output_select_pos____.txt', 'a', encoding='utf-8') as file2:
        fileline = 0
        for line1 in file1:
            fileline+=1
            print(f"7811-{fileline}")
            if fileline > 411:
                if contains_digit(line1):
                    file2.write(line1)
                else:
                    continue
            else:
                continue

def duplicate_removal():
    readlinelist = []
    with open('filtered_output_select.txt', 'r', encoding='utf-8') as file1, \
            open('filtered_output_select2.txt', 'a', encoding='utf-8') as file2:
        fileline = 0
        for line1 in file1:
            fileline += 1
            if line1 not in readlinelist:
                readlinelist.append(line1)
        print(fileline,'---',len(readlinelist))
        for line2 in readlinelist:
            file2.write(line2)


if __name__=='__main__':
    # getfile()
    # getkeywords()
    # getkeypos()
    # deletetext()
    # deletechinse()
    duplicate_removal()
