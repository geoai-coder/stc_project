...
def test01():
    ...
    with open('filtered_output_select3.txt', 'r', encoding='utf-8') as infile, open('filtered_output_select.txt', 'w', encoding='utf-8') as outfile:
        ...
        for line in infile:
            # 检查行中是否包含 '%' 或 '/'
            # if ('%' in line or '/' in line or '％' in line) and not contains_multiple_numbers(line):
            #     print(line.strip())  # 打印当前行内容
            #     i+=1
            #     continue
            if ('GDP、' in line and line.count('GDP、') == line.count('GDP') and line.count('GDP、') != line.count('元GDP') and line.count('生产总值') == 0):
                print(line.strip())  # 打印当前行内容
                i+=1
                continue
            outfile.write(line)
