    ...
    try:
        ...
        with open(input_file, 'r', encoding='utf-8') as infile,open(output_file, 'a', encoding='utf-8') as outfile:
            for line in infile:
                ...
                if rate > 0.24 and any(keyword in line for keyword in keywords):
                    outfile.write(line)
                print(f'第：{rate:.3%}')
...
def getkeywords():
    ...
    # 示例中文文档
    readnum,filelen = 0,475793
    with open('filtered_output_select.txt', 'r', encoding='utf-8') as file1,open('filtered_output_select_2.txt', 'w', encoding='utf-8') as file2:
        for line in file1:
            readnum += 1
            rate = readnum / filelen
            # 使用 TF-IDF 提取关键词
            keywords_tfidf = jieba.analyse.extract_tags(line, topK=5, withWeight=False)
            if any(element in keywords for element in keywords_tfidf) or any('人' in element for element in keywords_tfidf):
                file2.write(line)
                # print(line.strip())
...
def deletetext():
    with open('filtered_output_select_pos__.txt', 'r', encoding='utf-8') as file1,\
    open('filtered_output_select_pos___.txt', 'a', encoding='utf-8') as file2:
        fileline = 0
        for line1 in file1:
            fileline+=1
            print(f"7811-{fileline}")
...
def deletetext2():
    ...
    open('filtered_output_select2.txt', 'a', encoding='utf-8') as file2:
        ...
        for line1 in file1:
            ...
            if len(line1.strip()) < 50 and len(line1.strip()) > 20:
