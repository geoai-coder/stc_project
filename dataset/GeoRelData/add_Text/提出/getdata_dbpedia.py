...
def getfile():
    max_lines = 36606627
    # 定义参数
    def filter_and_split_file(input_file, output_prefix):
        try:
            ...
            with open(input_file, 'r', encoding='utf-8') as infile,open(output_file, 'a', encoding='utf-8') as outfile:
                for line in tqdm(infile,desc="getfile", total=max_lines, unit='line', unit_scale=True):
                    if any(keyword in line for keyword in keywords) and any(keyword in line for keyword in keywords1):
                        print(line)
                        outfile.write(line)
    ...
    filter_and_split_file(input_file, output_prefix)
    ...
    with open('filtered_output_select.txt', 'r', encoding='utf-8') as file1 ,\
        ...
        for line in tqdm(file1,desc="getkeypos", total=max_lines, unit='line', unit_scale=True):
            if any(keyword in line for keyword in keywords):
                ...
                if textpos != False and 'nh' in textpos:
                    file2.write(line)
                    # 提取 ns 词性对应的词并统计出现次数
...
def deletetext():
    ...
    open('filtered_output_select_pos__.txt', 'w', encoding='utf-8') as file2:
        fileline = 0
        for line1 in file1:
            fileline+=1
...
def deletetext2():
    ...
    open('filtered_output_select_pos___.txt', 'w', encoding='utf-8') as file2:
        fileline = 0
        for line1 in file1:
            fileline+=1
            if len(line1) < 20:
...
def duplicate_removal():
    readlinelist = []
    with open('filtered_output_select_pos___.txt', 'r', encoding='utf-8') as file1, \
